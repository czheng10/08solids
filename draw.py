...
def scanline_convert(polygons, i, screen, zbuffer ):
    ...
    dx0 = (xt - xb) / (yt - yb + 1.0)
    dx1 = (xm - xb) / (ym - yb + 1.0)
    dx1_1 = (xt - xm) / (yt - ym +1.0)
    dz0 = (zt - zb) / (yt - yb + 1.0)
    dz1 = (zm - zb) / (ym - yb + 1.0)
    dz1_1 = (zt - zm) / (yt - ym +1.0)

    #special triangles, no distinct middle
    if (yt == ym):
        draw_line(int(xt), int(yt), int(zt),
            int(xm), int(ym), int(zm), screen, zbuffer, color)
    if (yb == ym):
         draw_line(int(xb), int(yb), int(zb),
              int(xm), int(ym), int(zm), screen, zbuffer, color)
...
def draw_line( x0, y0, z0, x1, y1, z1, screen, zbuffer, color ):
    ...
    dz = (z1 - z0) / (loop_end - loop_start +1.0) #change in z
